...
class Bassist extends Musician {
  // Оголошуємо приватне поле для назви групи
  #band;

  // Конструктор приймає ім'я, інструмент і назву групи
  constructor(name, instrument, band) {
    // Викликаємо конструктор батьківського класу з ім'ям та інструментом
    super(name, instrument);
    // Присвоюємо значення назви групи приватному полю
    this.#band = band;
  }

  // Геттер для отримання значення назви групи
  get band() {
    return this.#band;
  }

  // Сеттер для зміни значення назви групи
  set band(newBand) {
    this.#band = newBand;
  }

  // Метод для приєднання до нової групи
  joinBand(newBand) {
    // Змінюємо назву групи
    this.#band = newBand;
  }

  // Перевизначений метод play() для відтворення музики басистом
  play() {
    // Виводимо рядок з ім'ям, інструментом та назвою групи, на якій він грає
    console.log(
      `${super.name} грає на ${super.instrument} в групі ${this.#band}`
    );
  }
...
class LeadSinger extends Vocalist {
  ...
  constructor(name, band) {
    // Викликаємо конструктор батьківського класу з параметрами name, band
    super(name, band);
  }
}
